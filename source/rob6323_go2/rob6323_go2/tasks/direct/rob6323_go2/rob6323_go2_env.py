...
class Rob6323Go2Env(DirectRLEnv):
    ...
    def __init__(self, cfg: Rob6323Go2EnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # adding handle for debug visualization (this is set to a valid handle inside set_debug_vis)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None: 
        ...
        # Computation of desired joint positions from policy actions
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # Torque magnitude penalty (∥τ∥) -very small reward scale
        rew_torque = torch.sum(torch.square(torques), dim=1) * -0.0001
        ...
        # Reward: square error weighted by swing phase (1 - desired_contact_states)
        rew_foot_clearance = torch.square(target_height - foot_height) * (1 - self.desired_contact_states)
        rew_feet_clearance = torch.sum(rew_foot_clearance, dim=1)

        # 2. Contact Force Reward
        # Get contact forces for feet using sensor indices
        contact_forces = self._contact_sensor.data.net_forces_w  # Shape: (num_envs, num_bodies, 3)
        foot_forces = torch.norm(contact_forces[:, self._feet_ids_sensor, :], dim=-1)  # Shape: (num_envs, 4)
        ...
        rew_tracking_contacts_shaped_force = rew_tracking_contacts_shaped_force / 4.0  # Average over feet
    ...
    def _set_debug_vis_impl(self, debug_vis: bool):
        ...
        if debug_vis:
            ...
            # setting their visibility to true
